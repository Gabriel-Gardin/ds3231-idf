...
DS3231_API int ds3231_initialize(ds3231_t driver, int *opt_out_osf)
{
    ...
    i2c_param_config(driver->port, &i2c_config);
    i2c_driver_install(driver->port, I2C_MODE_MASTER, 0, 0, 0);
...
DS3231_API int ds3231_get_temp_integer(ds3231_t driver, int16_t *out_temperature)
{
    ASSERT_DRV();
    ...
    *out_temperature = ((int16_t)(int8_t)tempr[0] << 2 | tempr[1] >> 6) >> 2;
